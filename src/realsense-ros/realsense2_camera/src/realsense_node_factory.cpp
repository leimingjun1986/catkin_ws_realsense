...
//重载 nodelet  onInit()函数   nodelet入口
void RealSenseNodeFactory::onInit()
{
	try
	{
		ros::NodeHandle nh = getNodeHandle();   //node 基类函数
		...
		if (!rosbag_filename.empty())   //从rosbag文件读取数据 不是从设备
		{
			{
			...
			_query_thread = std::thread([=]()
						{
							...
							while (_is_alive && !_device)      //类是否存在  初始话为true  析构false    设备不存在
							{
								getDevice(_ctx.query_devices());   //获取设备号
								if (_device)
								{
