      ...
      //cloud_out.points.assign (cloud_in.points.begin (), cloud_in.points.end ());
        ...
        int  num=src_pc.size();
         ...
         for(int i=0;i<num;i++)
         {
                ...
                Vector3f  point_temp;
               point_temp[0]  = static_cast<float> (transf(0,0)*point[0]+transf(0,1)*point[1]+transf(0,2)*point[2]+transf(0,3));
               point_temp[1]   = static_cast<float> (transf(1,0)*point[0]+transf(1,1)*point[1]+transf(1,2)*point[2]+transf(1,3));
               point_temp[2]  = static_cast<float> (transf(2,0)*point[0]+transf(2,1)*point[1]+transf(2,2)*point[2]+transf(2,3));

                //将世界坐标系变到机器人坐标系
                 dest_pc[i].x = base_to_world.at<float>(0,0)*point_temp[0] +    base_to_world.at<float>(0,1)*point_temp[1]  + base_to_world.at<float>(0,2)*point_temp[2]+base_to_world.at<float>(0,3);
                 dest_pc[i].y = base_to_world.at<float>(1,0)*point_temp[0] +    base_to_world.at<float>(1,1)*point_temp[1]  + base_to_world.at<float>(1,2)*point_temp[2]+base_to_world.at<float>(1,3);
                 dest_pc[i].z = base_to_world.at<float>(2,0)*point_temp[0]+    base_to_world.at<float>(2,1)*point_temp[1]  + base_to_world.at<float>(2,2)*point_temp[2]+base_to_world.at<float>(2,3);;
         }
}
...
void   vision_obstacle::pointcloud_obstacle( sensor_msgs::PointCloud2&  msg){
       ...
       // sd=ros::Time::now().toSec();
        //ROS_INFO_STREAM("PointCloud_DownSample time:" << (sd - st)*1000 );
         ...
         //st = ros::Time::now().toSec();
        PointCloud_CoordTranform( cloud, cloud,  transf); 
        //sd=ros::Time::now().toSec();
     ...
     //  st = ros::Time::now().toSec();
        if(model<=1)
        PointCloud_PassThrough(cloud,cloud);
        if(model>=2)
        PointCloud_PassThrough_Outdoor(cloud,cloud);
        //sd=ros::Time::now().toSec();
        //ROS_INFO_STREAM("PointCloud_PassThrough  time:" << (sd - st)*1000 );

        if(radis_rem_enable)
         PointCloud_RadiusFilter(cloud,cloud);    //球形滤波器
